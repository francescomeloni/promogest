...
class AnagraficaGruppoImballaggio(Anagrafica):
    ...
    def refresh(self):
        ...
        self.numRecords = GruppoImballaggio().count(denominazione=denominazione)

        self._refreshPageCount()

        # Let's save the current search as a closure
        def filterClosure(offset, batchSize):
            return GruppoImballaggio().select(denominazione=denominazione,
...
class AnagraficaGruppoImballaggioFilter(AnagraficaFilter):
    """ Filtro per la ricerca nell'anagrafica dei gruppo di imballaggio """
