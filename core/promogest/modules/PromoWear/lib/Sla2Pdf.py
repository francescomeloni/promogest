...
class Sla2Pdf:
    ...
    def __init__(self, slaFileName, pdfFolder, report):
        """
    ...
    def findTags(self, string):
        ...
        for e in elements:
            ...
            # Finding position
            indexPosition = e.find(':')
            if indexPosition != -1:
                position = e[2:indexPosition]

            # Analyzing tag
            if e.count('(') != e.count(')'):
                continue

            indexBeginFunction = e.find('(')
            indexEndFunction = e.rfind(')')
            isIterator = e.find('(n)')
            if (((isIterator == -1) or ((isIterator != -1) and (isIterator != indexBeginFunction))) and
                (indexBeginFunction != -1) and (indexEndFunction != -1)):
                    # Finding function
                    if indexPosition != -1:
                        function = e[indexPosition+1:indexBeginFunction]
                    else:
                        function = e[2:indexBeginFunction]

                    # Finding tag and parameter
                    indexParameter = e.find(',')
                    if indexParameter != -1:
                        parameter = e[indexParameter+1:indexEndFunction]
                        tag = e[indexBeginFunction+1:indexParameter]
                    else:
                        tag = e[indexBeginFunction+1:indexEndFunction]

            # Finding tag
            else:
                if indexPosition != -1:
                    tag = e[indexPosition+1:-2]
                else:
                    tag = e[2:-2]
        ...
        for j in range(1, len(numPages)):
            for pageObject in self.pageObjects:
                ...
                for itext in itexts:
                    ...
                    # Applying attributes
                    SubElement(app, 'ITEXT', dictionary)
                ...
                if not(len(itexts) > 0):
                    ...
                    for k in tagsKeys:
                        ...
                        if k.find('(n)') > -1:
                            ...
                            increment = False
                    itext['CH'] = tmp
                    SubElement(pageObject, 'ITEXT', itext)
                else:
