...
DROP VIEW v_stoccaggio;

CREATE OR REPLACE VIEW v_stoccaggio AS
    SELECT 
         S.id
        ,S.scorta_minima
        ,S.livello_riordino
        ,S.data_fine_scorte
        ,S.data_prossimo_ordine
        ,S.id_articolo
        ,S.id_magazzino
    FROM stoccaggio S;


DROP VIEW v_stoccaggio_completa;

CREATE OR REPLACE VIEW v_stoccaggio_completa AS
    SELECT 
         S.id
        ,S.scorta_minima
        ,S.livello_riordino
    ...
    FROM stoccaggio S
    LEFT OUTER JOIN articolo A ON S.id_articolo = A.id
    LEFT OUTER JOIN magazzino M ON S.id_magazzino = M.id
