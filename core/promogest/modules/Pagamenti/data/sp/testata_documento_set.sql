...
DROP FUNCTION promogest.TestataDocumentoInsUpd(varchar, bigint, bigint, integer, integer, varchar, varchar, date, timestamp, varchar, text, varchar, varchar, varchar, timestamp, timestamp, varchar, integer, varchar, varchar, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, varchar, boolean, decimal(16,4), decimal(16,4), bigint, bigint, decimal(16,4), boolean);
CREATE OR REPLACE FUNCTION promogest.TestataDocumentoInsUpd(varchar, bigint, bigint, integer, integer, varchar, varchar, date, timestamp, varchar, text, varchar, varchar, varchar, timestamp, timestamp, varchar, integer, varchar, varchar, bigint, bigint, bigint, bigint, bigint, bigint, bigint, bigint, varchar, boolean, decimal(16,4), decimal(16,4), bigint, bigint, decimal(16,4), boolean) RETURNS promogest.resultid AS E'
    DECLARE
        -- Parametri contesto
        _schema                     ALIAS FOR $1;
    ...
    BEGIN
        IF _id IS NULL THEN
            INSERT INTO testata_documento (numero,parte,registro_numerazione,operazione,data_documento,data_inserimento,protocollo,note_interne,note_pie_pagina,causale_trasporto,aspetto_esteriore_beni,inizio_trasporto,fine_trasporto,incaricato_trasporto,totale_colli,totale_peso,applicazione_sconti,id_cliente,id_fornitore,id_destinazione_merce,id_pagamento,id_banca,id_aliquota_iva_esenzione,id_vettore,id_agente,porto, costo_da_ripartire, ripartire_importo) 
                VALUES (_numero,_parte,_registro_numerazione,_operazione,_data_documento,CURRENT_TIMESTAMP,_protocollo,_note_interne,_note_pie_pagina,_causale_trasporto,_aspetto_esteriore_beni,_inizio_trasporto,_fine_trasporto,_incaricato_trasporto,_totale_colli,_totale_peso,_applicazione_sconti,_id_cliente,_id_fornitore,_id_destinazione_merce,_id_pagamento,_id_banca,_id_aliquota_iva_esenzione,_id_vettore,_id_agente,_porto, _costo_da_ripartire, _ripartire_importo);
            ...
            VALUES (_documento_saldato, TempId, _id_primo_riferimento, _id_secondo_riferimento, _totale_pagato, _totale_sospeso);
        ...
        ELSE
            UPDATE testata_documento SET
                ...
                ,porto = _porto
                ,costo_da_ripartire = _costo_da_ripartire
                ,ripartire_importo = _ripartire_importo
            WHERE id = _id;
            TempId :=  id_documento from informazioni_contabili_documento WHERE ( id_documento = _id);
