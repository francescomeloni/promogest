...
#        self._idMagazzino = idMagazzino
    ...
    def calcolo_ricarico_medio_e_influenza_sulle_vendite(self):
        ...
        if self.cateArticolo[3]:
            catesArt = CategoriaArticolo().select(batchSize=None)
        else:
            catesArt = self.cateArticolo[1]
                ...
                for rig in righeArticoloMovimentate:
                    ...
                    if not rigaArticoloMovimentata:
                        ...
                        # for r in rigaArticoloMovimentata:
                            valoreAcquisto += (rigaArticoloMovimentata[0].valore_unitario_netto*quantitaVendutaUNO)
                            valoreAcquistoTotale += (rigaArticoloMovimentata[0].valore_unitario_netto*quantitaVendutaUNO)

                            quantitaAcquistataUNO += rigaArticoloMovimentata[0].quantita
                            quantitaAcquistataTotaleUNO += rigaArticoloMovimentata[0].quantita
    #                        quantitaAcquistataTotale += r[0].quantita
                    ...
                    # devo scorporare l'iva dal prezzo finale di vendita
                        imponibile = Decimal(str(float(rig.valore_unitario_netto)/(1+float(rig.percentuale_iva)/100)))
                    elif ope["fonteValore"] =="vendita_senza_iva":
                        imponibile = Decimal(str(float(rig.valore_unitario_netto)))
                    else:
