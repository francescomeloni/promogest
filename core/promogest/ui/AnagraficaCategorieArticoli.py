...
class AnagraficaCategorieArticoli(Anagrafica):
    ...
    def refresh(self):
        ...
        self.numRecords = CategoriaArticolo(isList=True).count(denominazione=denominazione)

        self._refreshPageCount()

        # Let's save the current search as a closure
        def filterClosure(offset, batchSize):
            return CategoriaArticolo(isList=True).select(denominazione=denominazione,
...
class AnagraficaCategorieArticoliDetail(AnagraficaDetail):
    ...
    def setDao(self, dao):
        if dao is None:
            self.dao = CategoriaArticolo().getRecord()
    ...
    def updateDao(self):
        self.dao = CategoriaArticolo(id=self.dao.id).getRecord()
        self._refresh()
