...
def anagraficaVettore(req, action=None):
    ...
    def _list_(req, action=None):
        ...
        batch = 20
        count = Vettore(req=req).count(ragioneSociale=fk_ragione_sociale,
                                        insegna=fk_insegna,
        ...
        args["page_list"] = "anagrafiche/vettore/list"
        daos = Vettore(req=req).select( ragioneSociale=fk_ragione_sociale,
                                        insegna=fk_insegna,
                                        cognomeNome=fk_cognome_e_nome,
                                        codice = fk_codice,
                                        localita = fk_localita,
    ...
    def __add__(req, action=None):
        ...
        if action == "add":
            ...
            dao = Vettore().getRecord(id=id)
        codice = req.form.get("codice")
        if codice:
            dao.codice = codice
        else:
            dao.codice = getNuovoCodiceVettore()
        ...
        if dao.codice:
            dao.persist()
        redirectUrl='/anagrafiche/vettore/list'
    ...
    def __del__(req, action=None):
        ...
        if dao:
            dao.delete()
        daos = Vettore(req=req).select()
        pageData = {'file' : "/tree/treeVettore",
                    "_dao_":"vettore",
                    "name": "Vettore",
                    "tree":"treeVettore",
                    "action":action,
    ...
    def __edit__(req, action=None):
        id = req.form.get("idr")
        dao = Vettore().getRecord(id=id)
        pageData = {'file' : "/addedit/ae_vettore",
                    "_dao_":"vettore",
                    "name": "Vettore",
                    "tree":"treeVettore",
                    "action":action,
